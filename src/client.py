...
from errors import ImproperlyConfigured,UserNotConsented,DelegateStateError
from jwt import build_jwt
from crypto import (
    SerializedKey,
...
class CHClient():
    ...
    def perform_initialization(
        ...
        name,
        logo_uri,
        clear_existing_signing_keys
    ):
        existing_config = self.get_configuration()

        # check if signing key needs to be bootstrapped
        if not existing_config['signing_keys'] or clear_existing_signing_keys==True:
            if self.logging_enabled:
        ...
        if name != existing_config["name"]:
            update['name'] = name
            logger.info(f'Updating partner configuration with new name: {name}')
        if logo_uri != existing_config["logo_uri"]:
            update["logo_uri"] = logo_uri
            logger.info(f'Updating partner configuration with new logo URI: {logo_uri}')
